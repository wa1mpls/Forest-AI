...
from models.spectral_attention import SpectralAttention
import unittest
...
class TestSpectralAttention(unittest.TestCase):
    def setUp(self):
        ...
        self.height = 224
        self.width = 224
        self.model = SpectralAttention(num_bands=self.num_bands)
    ...
    def test_forward_shape(self):
        ...
        # Check output shape
        self.assertEqual(output.shape, (self.batch_size, self.num_bands, self.height, self.width))
    ...
    def test_training(self):
        ...
        x = torch.randn(self.batch_size, self.num_bands, self.height, self.width)
        y = torch.randn(self.batch_size, self.num_bands, self.height, self.width)
    ...
    def test_save_load(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pt', delete=False) as tmp:
            ...
            # Load model
            new_model = SpectralAttention(num_bands=self.num_bands)
    ...
    def test_attention_weights(self):
        ...
        self.assertTrue(torch.allclose(attention_weights.sum(dim=1), torch.ones_like(attention_weights.sum(dim=1))))
